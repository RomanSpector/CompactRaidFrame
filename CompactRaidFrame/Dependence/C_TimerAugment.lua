...
--]]
C_Timer = CreateFrame('Frame')
C_Timer.After = function(duration, callback)
  if not duration or not callback then return end
  if duration <= 0 then return callback() end -- Animation does not start when the duration <= 0

  local handler = C_Timer:CreateAnimationGroup()
  local timer = handler:CreateAnimation()
...
-- this code from C_TimerAugment.lua (7.3.5.25864)
